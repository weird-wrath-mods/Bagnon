...
function Frame:Layout()
	...
	local w, h = self:PlaceSortToggle()
	width = width + w
	height = height + 20
...
function Frame:GetSortToggle()
	return self.sortToggle
end

function Frame:CreateSortToggle()
	local b = Bagnon.SortToggle:New(self:GetFrameID(), self)
	self.sortToggle = b
	return b
end

function Frame:HasSortToggle()
	local frameID = self:GetFrameID()
	return frameID == 'inventory' or frameID == 'bank'
end

function Frame:PlaceSortToggle()
	if self:HasSortToggle() then
		local b = self:GetSortToggle() or self:CreateSortToggle()
		b:ClearAllPoints()
		if self:HasOptionsToggle() then
...
function Frame:GetProspectButton()
	return self.prospectButton
end

function Frame:CreateProspectButton()
	local b = Bagnon.ProspectButton:New(self:GetFrameID(), self)
	self.prospectButton = b
	return b
end

function Frame:HasProspectButton()
	if self:GetFrameID() ~= 'inventory' then return false end
	return Bagnon.ProspectButton and Bagnon.ProspectButton.PlayerKnows()
end
